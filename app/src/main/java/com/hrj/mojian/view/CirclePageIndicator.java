...
public class CirclePageIndicator extends View implements PageIndicator {
	...
	public CirclePageIndicator(Context context, AttributeSet attrs, int defStyle) {
		...
		final int defaultSpace = res
				.getInteger(R.integer.default_circle_indicator_space);
		final boolean defaultTouch = res
				.getBoolean(R.bool.default_circle_indicator_touch);
		...
		mSpace = a.getInt(R.styleable.CirclePageIndicator_space, defaultSpace);
		mTouch = a.getBoolean(R.styleable.CirclePageIndicator_touch,
				defaultTouch);
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mCentered) {
			longOffset += ((longSize - longPaddingBefore - longPaddingAfter) / 2.0f)
					- ((count * 2 * mRadius + (count - 1) * threeRadius) / 2.0f);
		}
		...
		for (int iLoop = 0; iLoop < count; iLoop++) {
			float drawLong = longOffset + (iLoop * threeRadius) + iLoop * 2
					* mRadius;
			if (mOrientation == HORIZONTAL) {
				dX = drawLong;
				dY = shortOffset;
		...
		int curPage = mSnap ? mSnapPage : mCurrentPage;
		float cx = curPage * 2 * mRadius + threeRadius * curPage;
		
		if (!mSnap) {
			cx += mPageOffset * (threeRadius + 2 * mRadius);
		}
		if (mOrientation == HORIZONTAL) {
			dX = longOffset + cx;
			dY = shortOffset;
		...
		case MotionEvent.ACTION_UP:
			if (!mIsDragging) {
				...
				if ((mCurrentPage > 0) && (ev.getX() < halfWidth - sixthWidth)) {
					if (action != MotionEvent.ACTION_CANCEL && mTouch) {
