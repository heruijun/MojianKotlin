 ...
 */
public abstract class MJOrientationEventListener {
	private static final String TAG = "OrientationEventListener";
	private static final boolean DEBUG = false;
	private static final boolean localLOGV = false;
	private int mOrientation = ORIENTATION_UNKNOWN;
	private boolean mWard = true;
	 ...
	 */
	public MJOrientationEventListener(Context context) {
	 ...
	 *            detection.
	 */
	public MJOrientationEventListener(Context context, int rate) {
	...
	/**
	 * Enables the OrientationEventListener so it will monitor the sensor and
	 ...
	 * parameter is in degrees, ranging from 0 to 359. orientation is 0 degrees
	 * when the device is oriented in its natural position, 90 degrees when its
	 * left side is at the top, 180 degrees when it is upside down, and 270
	 * degrees when its right side is to the top. {@link #ORIENTATION_UNKNOWN}
	 * is returned when the device is close to flat and the orientation cannot
	 * be determined.
	 * 
	 * @param orientation
	 ...
	 * 
	 * @param ward
	 *            true is screen forward ,false is screen backward
	 */
	abstract public void onWardChanged(boolean ward);
}
